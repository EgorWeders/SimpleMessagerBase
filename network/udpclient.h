...
private slots:
  void acceptData() {
    ...
    qDebug() << "Recv id" << msg.id() << "body"
             << QString::fromStdString(msg.stringBody());
  };
  void sendData(){};

public:
  UdpClient(const QString &addr, int port = DEFAULT_PORT)
      : socket(this), host(addr), port(port) {
    bind();
...
public:
  bool sendMessage() override {
    ...
    qDebug() << "Sending id <" << tempMsgd.id() << "> body <"
             << QByteArray(tempMsgd.body(), tempMsgd.bodySize()) << "> to"
             << target << targetPort;
    return socket.writeDatagram(
        QByteArray(tempMsgd.data(), tempMsgd.dataSize()), target, targetPort);
  }
};
